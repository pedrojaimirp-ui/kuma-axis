-- 0026_giros_infinitos_owner.sql --------------------------------------------
-- El dueño/admin necesita girar la ruleta todos los días para hacer
-- demostraciones, sin que se le agoten los giros. Esta version de
-- spin_roulette() detecta si quien gira es admin/owner y, en ese caso,
-- NUNCA descuenta giros (quedan infinitos para esa cuenta). El resto de
-- usuarios sigue funcionando exactamente igual que antes.
-- -----------------------------------------------------------------------------
...
declare
  ...
  v_amount   numeric;
begin
  select (role in ('admin', 'owner')) into v_is_staff
  ...
  if v_amount > 0 then
    ...
    -- "Vuelve y juega" devuelve 1 giro extra (solo aplica a usuarios normales)
    update public.spin_credits
